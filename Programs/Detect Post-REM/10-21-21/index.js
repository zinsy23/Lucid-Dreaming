...
let timesAsleep = 0; //////////////////
let secondCheck; ///////////////
let startDelay = setInterval(delayStart, 9000000);
let secondFix;
...
function addData(){
  ...
  numbers[j] += "    " + getTime() + "; " + hrm.heartRate + "; " + total + "; " + avgHeartRate() + "\n";
  updateData();
  totalMotion += isREM ? total : 0;
  total = 0;
  if(dataPoint >= 1300) stopData();
  dataPoint++;
  let now = new Date();
...
function avgHeartRate(){
  ...
  hrIndex++;
  console.log(isREM + "; " + totalMotion);
  if(totalMotion > 1000 && totalREM >= 15){
    // Alarm may go off!!!!!!
  ...
  if(hrIndex >= 10 && !isREM){
    ...
    hrIndex = 0;
    totalNREM++;
    if(avgHr > resting && totalNREM >= 6){
      isREM = true;
      intREM = setInterval(getREM, 300000);
    }
    return avgHr > resting ? avgHr + "a" : 0; // Checking if hr is above resting but just to check "analyze" if misssing "REM"
